...
void HumanoidBody::Update()
{
	...
	parts_[(uint32_t)PartID::Head]->Update(&transform);
...
void HumanoidBody::DrawModel()
{
	...
	//parts_[(uint32_t)PartID::RightArm]->Draw();
	//parts_[(uint32_t)PartID::RightHand]->Draw();
}
...
void HumanoidBody::DrawDebugGui()
{
	...
	GuiManager::DrawSlider3("Head Rot", parts_[(uint32_t)PartID::Head]->rot, move);
...
void HumanoidBody::JoggingMotion()
{
	...
	if (isReverce == true)
	{
		...
		// ‰E˜r
		parts_[(uint32_t)PartID::RightArm]->rot.x = joggingEase.InOut(Radian(25), Radian(-5));
		// ¶˜r
		parts_[(uint32_t)PartID::LeftArm]->rot.x = joggingEase.InOut(Radian(-5), Radian(25));

		// ‰E‘«
		parts_[(uint32_t)PartID::RightThigh]->rot.x = joggingEase.InOut(Radian(-40), Radian(30));
		parts_[(uint32_t)PartID::RightLeg]->rot.x = joggingEase.InOut(Radian(30), Radian(10));
		// ¶‘«
		parts_[(uint32_t)PartID::LeftThigh]->rot.x = joggingEase.InOut(Radian(30), Radian(-40));
	...
	else if (isReverce == false)
	{
		...
		// ‰E˜r
		parts_[(uint32_t)PartID::RightArm]->rot.x = joggingEase.InOut(Radian(-5), Radian(25));
		// ¶˜r
		parts_[(uint32_t)PartID::LeftArm]->rot.x = joggingEase.InOut(Radian(25), Radian(-5));

		// ‰E‘«
		parts_[(uint32_t)PartID::RightThigh]->rot.x = joggingEase.InOut(Radian(30), Radian(-40));
		parts_[(uint32_t)PartID::RightLeg]->rot.x = joggingEase.InOut(Radian(10), Radian(30));
		// ¶‘«
		parts_[(uint32_t)PartID::LeftThigh]->rot.x = joggingEase.InOut(Radian(-40), Radian(30));
